...
export const ANIM_PROPS = [
  { key: 'px', label: 'Geser X', unit: 'len', def: 0 },
  { key: 'py', label: 'Geser Y', unit: 'len', def: 0 },
  { key: 'pz', label: 'Geser Z', unit: 'len', def: 0 },
  { key: 'rx', label: 'Putar X', unit: 'ang', def: 0 },
  { key: 'ry', label: 'Putar Y', unit: 'ang', def: 0 },
  { key: 'rz', label: 'Putar Z', unit: 'ang', def: 0 },
  { key: 'sx', label: 'Skala X', unit: 'num', def: 1 },
  { key: 'sy', label: 'Skala Y', unit: 'num', def: 1 },
  { key: 'sz', label: 'Skala Z', unit: 'num', def: 1 },
  { key: 'op', label: 'Opasitas', unit: 'num', def: 1 },
  { key: 'vis', label: 'Terlihat', unit: 'bool', def: 1 },
];
...
export const SCHEDULE_MODES = [
  ['none', 'Muncul'],
  ['fade', 'Memudar masuk'],
  ['grow', 'Tumbuh dari tengah'],
  ['riseZ', 'Naik (Z)'],
  ['dropZ', 'Jatuh masuk (Z)'],
  ['slideX', 'Geser masuk (X)'],
  ['slideY', 'Geser masuk (Y)'],
  ['build', 'Tumbuh bertahap (sapuan Z)'],
];

export const MOTOR_TYPES = [
  ['none', 'Tidak ada'],
  ['spin', 'Putar terus'],
  ['oscillate', 'Ayun (rotari)'],
  ['reciprocate', 'Bolak-balik (linier)'],
  ['orbit', 'Orbit sebuah titik'],
];
...
export class Simulator {
  ...
  setKey(featureId, prop, t, value, ease = 'smooth') {
    store.edit('Pasang keyframe', (doc) => {
  ...
  bakeToKeys(step = 4) {
    this.ensureBaked();
    if (!this.frames) { bus.emit(T.TOAST, { msg: 'Aktifkan dinamika dulu', kind: 'warn' }); return 0; }
    const fps = this.bakeFps;
    let n = 0;
    store.edit('Bake dinamika ke keyframe', (doc) => {
  ...
  autoSchedule({ perItem = 1, gap = 0.25, mode = 'grow' } = {}) {
    ...
    store.edit('Jadwalkan otomatis urutan bangun', (doc) => {
