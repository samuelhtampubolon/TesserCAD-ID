...
export const DRAW_TOOLS = [
  { id: 'select', label: 'Pilih', glyph: '⬈', key: 'Esc' },
  { id: 'line', label: 'Baris', glyph: '╱', key: 'L' },
  ...
  { id: 'text', label: 'Teks', glyph: 'T', key: 'X' },
  { id: 'dimLinear', label: 'Dim linear', glyph: '↔', key: 'D' },
  { id: 'dimAligned', label: 'Dim sejajar', glyph: '⤡', key: '' },
  { id: 'dimRadial', label: 'Dim radius', glyph: '◠', key: '' },
  { id: 'dimAngular', label: 'Dim sudut', glyph: '∠', key: '' },
  { id: 'offset', label: 'Offset', glyph: '⧉', key: 'O' },
  { id: 'measure', label: 'Ukur', glyph: '⟺', key: 'M' },
];
...
export class Draft2D {
  ...
  _toolClick(p) {
    ...
    switch (t) {
      ...
      case 'dimRadial': {
        ...
        } else bus.emit(T.TOAST, { msg: 'Klik lingkaran atau arc untuk didimensikan', kind: 'warn' });
        break;
      }
      ...
      case 'offset': {
        if (!this.pending.length) {
          const hit = this.hitTest(p);
          if (!hit) { bus.emit(T.TOAST, { msg: 'Pilih objek untuk di-offset', kind: 'warn' }); return; }
          this.pending = [p];
          ...
          if (copy) this._add(copy); else bus.emit(T.TOAST, { msg: 'Objek itu tidak bisa di-offset', kind: 'warn' });
          this.pending = [];
          this.offsetSrc = null;
        }
        break;
      ...
      case 'measure':
        ...
        if (this.pending.length === 2) {
          ...
          const d = dist(a, b);
          bus.emit(T.TOAST, {
            msg: `Jarak ${fmt(toDisplay(d, u))} ${u} · ΔX ${fmt(toDisplay(b[0] - a[0], u))} · ΔY ${fmt(toDisplay(b[1] - a[1], u))} · ∠ ${fmt(Math.atan2(b[1] - a[1], b[0] - a[0]) * R2D)}°`,
            kind: 'ok', ms: 6000,
          });
          this.pending = [];
        }
  ...
  _add(entity) {
    ...
    store.edit(`Gambar ${entity.type}`, (doc) => { doc.draw.entities.push(e); }, { rebuild: true });
...
const TOOL_PROMPTS = {
  select: 'Klik untuk memilih · seret kanan-ke-kiri untuk jendela silang · Del menghapus',
  line: (n) => n ? 'Pilih titik akhir (atau ketik panjang)' : 'Pilih titik awal',
  polyline: (n) => n ? 'Pilih titik berikutnya · Enter selesai · C menutup' : 'Pilih titik awal',
  spline: (n) => n ? 'Pilih titik fit berikutnya · Enter selesai' : 'Pilih titik fit pertama',
  rect: (n) => n ? 'Pilih sudut yang berseberangan' : 'Pilih sudut pertama',
  circle: (n) => n ? 'Pilih titik pada lingkaran (atau ketik radius)' : 'Pilih titik pusat',
  ellipse: (n) => n === 0 ? 'Pilih titik pusat' : n === 1 ? 'Pilih radius X' : 'Pilih radius Y',
  polygon: (n) => n ? 'Pick a vertex (radius + rotation)' : 'Pilih titik pusat',
  arc: (n) => n === 0 ? 'Pilih awal arc' : n === 1 ? 'Pilih titik di sepanjang arc' : 'Pilih akhir arc',
  point: 'Klik untuk menempatkan node',
  text: 'Klik untuk menempatkan teks',
  dimLinear: (n) => n < 2 ? isi('Pilih titik ukur {n}', { n: n + 1 }) : 'Pilih posisi garis dimensi',
  dimAligned: (n) => n < 2 ? isi('Pilih titik ukur {n}', { n: n + 1 }) : 'Pilih posisi garis dimensi',
  dimRadial: 'Klik lingkaran atau arc',
  dimAngular: (n) => n === 0 ? 'Pilih titik sudut' : n === 1 ? 'Pilih kaki pertama' : 'Pilih kaki kedua',
  offset: (n) => n ? 'Klik sisi tujuan offset' : 'Pilih objek untuk di-offset',
  measure: (n) => n ? 'Pilih titik kedua' : 'Pilih titik pertama',
};
