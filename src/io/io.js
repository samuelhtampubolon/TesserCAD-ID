...
export function download(filename, data, mime = 'application/octet-stream') {
  ...
  bus.emit(T.TOAST, { msg: `Tersimpan ${filename}`, kind: 'ok' });
}
...
export async function openProjectFile(file) {
  ...
  bus.emit(T.TOAST, { msg: `Dibuka ${file.name}`, kind: 'ok' });
}
...
export function exportGroup(viewport) {
  ...
  for (const { mesh, feature } of collectMeshes(viewport)) {
    ...
    m.name = (feature?.name || 'Badan').replace(/\s+/g, '_');
    root.add(m);
  }
  return root;
}
...
export function exportSTL(viewport, { binary = true } = {}) {
  ...
  if (!root.children.length) { bus.emit(T.TOAST, { msg: 'Tidak ada yang bisa diekspor', kind: 'warn' }); return; }
...
export function exportPLY(viewport) {
  ...
  if (!meshes.length) { bus.emit(T.TOAST, { msg: 'Tidak ada yang bisa diekspor', kind: 'warn' }); return; }
  const verts = [];
  const cols = [];
...
export function exportDXF() {
  if (!store.doc.draw.entities.length) { bus.emit(T.TOAST, { msg: 'Gambarnya kosong', kind: 'warn' }); return; }
...
export function exportPNG(viewport, scale = 2) {
  ...
  fetch(url)
    ...
    .catch(err => bus.emit(T.TOAST, { msg: isi('Ekspor PNG gagal: {error}', { error: err.message || err }), kind: 'err' }));
}

/** A plain-text bill of materials for the current model. */
export function exportBOM(build) {
  const rows = [['No.', 'Fitur', 'Jenis', 'Material', 'Badan', 'Volume (mm^3)', 'Massa (kg)']];
  let i = 1;
...
export async function importMeshFile(file) {
  ...
  if (ext === 'stl') {
    ...
    throw new Error(isi('Format mesh “.{ext}” tidak didukung', { ext }));
  }
  if (!geometries.length) throw new Error('Tidak ditemukan geometri di berkas itu');

  const added = [];
  store.edit(`Impor ${file.name}`, (doc) => {
  ...
  bus.emit(T.TOAST, { msg: isi('{n} body diimpor dari {file}', { n: geometries.length, file: file.name }), kind: 'ok' });
  return added;
}
...
export async function importDXFFile(file, { replace = false } = {}) {
  ...
  if (!parsed.entities.length) throw new Error('Tidak ditemukan entitas yang bisa digambar di DXF itu');
  store.edit(`Impor ${file.name}`, (doc) => {
  ...
  bus.emit(T.TOAST, { msg: isi('{n} entitas diimpor dari {file}', { n: parsed.entities.length, file: file.name }), kind: 'ok' });
