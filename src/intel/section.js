...
export const LOAD_CASES = {
  cantilever: {
    label: 'Kantilever, beban di ujung',
    note: 'Jepit di satu ujung, beban di ujung bebas. Lentur terburuk ada di ujung jepit, dan itulah penampang yang diperiksa.',
    moment: (F, L) => F * L,
  },
  simply: {
    label: 'Tumpuan sederhana, beban di tengah bentang',
    note: 'Ditumpu di kedua ujung, beban di tengah. Lentur terburuk ada di tengah bentang.',
    moment: (F, L) => (F * L) / 4,
  },
  fixedBoth: {
    label: 'Jepit di kedua ujung, beban di tengah bentang',
    note: 'Kedua ujung dijepit. Lebih kaku daripada tumpuan sederhana, dan lentur terburuk ada di tumpuan.',
    moment: (F, L) => (F * L) / 8,
  },
  axial: {
    label: 'Beban aksial saja',
    note: 'Tarik atau tekan murni sepanjang batang. Tanpa lentur sama sekali.',
    moment: () => 0,
  },
};
...
export function checkSection(section, { case: kind = 'cantilever', force = 0, span = 100, material = 'aluminium', safety = 2 } = {}) {
  ...
  return {
    ...
    verdict: total <= allow
      ? isi('{p1}% dari yang diizinkan', { p1: ((total / allow) * 100).toFixed(0) })
      : isi('lewat {p1}%', { p1: ((total / allow - 1) * 100).toFixed(0) }),
  };
}
...
export function standardPlanes(box) {
  ...
  return {
    xy: { label: 'Mendatar (XY)', plane: new THREE.Plane(new THREE.Vector3(0, 0, 1), -c.z) },
    xz: { label: 'Depan (XZ)', plane: new THREE.Plane(new THREE.Vector3(0, 1, 0), -c.y) },
    yz: { label: 'Samping (YZ)', plane: new THREE.Plane(new THREE.Vector3(1, 0, 0), -c.x) },
  };
}
