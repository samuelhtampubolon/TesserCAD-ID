...
export function toSpec(doc) {
  ...
  w(`part ${quote(doc.meta?.name || 'Tanpa judul')}`);
  ...
  if (doc.params?.length) {
    w('');
    w('# Parameter. Field fitur mana pun bisa merujuk ini dengan namanya.');
  ...
  for (const f of doc.features || []) {
    ...
    if (!cat) lossy.push({ feature: f.name, what: 'type', note: isi('Tipe fitur "{type}" tidak dikenal; ia tetap ditulis tetapi tidak akan dibangun ulang.', { type: f.type }) });
    ...
    if (f.data?.positions?.length) {
      ...
      w(isi('{INDENT}mesh {n} triangles   # muatan disimpan di dokumen, bukan di teks', { INDENT, n }));
      lossy.push({ feature: f.name, what: 'mesh', note: isi('{n} segitiga tetap melekat pada dokumen; teksnya hanya mencatat bahwa mereka ada.', { n }) });
    }
  }
  w('');
...
export function fromSpec(text, { base = null, keepIds = true } = {}) {
  ...
  const doc = newDocument('Tanpa judul');
  doc.params = [];
  doc.features = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (indented) {
      if (!current) { fail(i, 'Baris menjorok tanpa fitur di atasnya.'); continue; }
      ...
      if (!key) { fail(i, 'Kunci sebelum "=" tidak ada.'); continue; }
      ...
      if (key === 'pos' || key === 'rot' || key === 'scale') {
        ...
        if (a.length !== 3 || !usable) { fail(i, isi('{key} butuh tiga komponen, masing-masing sebuah angka atau ekspresi.', { key })); continue; }
        current.transform[key] = a;
      } else if (key === 'material') {
        const m = String(val);
        if (!MATERIALS[m]) warnings.push({ line: i + 1, message: isi('Material "{material}" tidak dikenal; kembali ke baja.', { material: m }) });
        ...
        warnings.push({ line: i + 1, message: isi('"{key}" adalah kata cadangan dan diabaikan.', { key }) });
      } else {
        current.params[key] = val;
      }
      continue;
    ...
    if (!m) { fail(i, isi('Tidak bisa membaca "{line}".', { line })); continue; }
    const [, word, rest] = m;

    switch (word) {
      case 'part': doc.meta.name = unquote(rest) || 'Tanpa judul'; break;
      case 'units': {
        const u = unquote(rest);
        if (!UNITS[u]) { warnings.push({ line: i + 1, message: isi('Satuan "{u}" tidak dikenal; tetap memakai mm.', { u }) }); break; }
        doc.meta.units = u;
        break;
      }
      ...
      case 'param': {
        ...
        if (eq < 0) { fail(i, 'Parameter butuh nilai: param nama = 10.'); break; }
        const name = unquote(rest.slice(0, eq));
        if (!isIdent(name)) { fail(i, isi('"{name}" bukan nama parameter yang bisa dipakai. Huruf, angka, dan garis bawah, diawali huruf.', { name })); break; }
        if (RESERVED_NAMES.has(name)) { fail(i, isi('"{name}" adalah kata cadangan bahasa ini dan tidak bisa menjadi nama parameter.', { name })); break; }
        if (seenParams.has(name)) { fail(i, isi('Parameter "{name}" didefinisikan dua kali.', { name })); break; }
        seenParams.add(name);
      ...
      case 'feature': {
        ...
        if (!fm) { fail(i, 'Fitur butuh tipe dan nama: feature box "Pelat".'); break; }
        const type = fm[1];
        if (!CATALOG[type]) { fail(i, isi('Tipe fitur "{type}" tidak dikenal. Tipe yang dikenal: {p1}, ...', { type, p1: Object.keys(CATALOG).slice(0, 8).join(', ') })); break; }
        const name = fm[2] ? unquote(fm[2]) : CATALOG[type].label;
        const wantId = keepIds && fm[3] ? fm[3] : null;
        if (wantId && seenIds.has(wantId)) { fail(i, isi('Dua fitur memakai id #{wantId} yang sama.', { wantId })); break; }
      ...
      default:
        fail(i, isi('"{word}" bukan kata kunci spec. Yang diharapkan: part, units, author, notes, param, atau feature.', { word }));
    }
  }
  ...
  for (const f of doc.features) {
    ...
    for (const [k, v] of Object.entries(cat?.params || {})) {
      if (!(k in f.params)) {
        ...
        warnings.push({ line: 0, message: isi('{feature}: {field} tidak diberikan, memakai bawaan {value}.', { feature: f.name, field: k, value: value(v) }) });
      }
    }
  }
  ...
  for (const f of doc.features) {
    const check = (label, v) => {
      ...
      if (!r.ok) warnings.push({ line: 0, message: isi('{feature}: {field} "{value}" tidak bisa dihitung ({error}).', { feature: f.name, field: label, value: v, error: r.error }) });
    };
    const cat = CATALOG[f.type];
  ...
  for (const f of doc.features) {
    ...
    if (missing.length) errors.push({ line: 0, text: f.name, message: isi('{feature} merujuk ke {refs}, yang tidak didefinisikan fitur mana pun.', { feature: f.name, refs: missing.map(x => '#' + x).join(', ') }) });
  }
...
export function roundTrip(doc) {
  ...
  if (!back) return { ok: false, text, errors, warnings, lossy, differences: ['Spesifikasinya gagal diurai.'] };
  ...
  const walk = (x, y, path) => {
    ...
    differences.push(`${path}: ${JSON.stringify(x)} menjadi ${JSON.stringify(y)}`);
  };
  walk(a, b, '');
...
export function reviewSpec(text, doc) {
  ...
  if (!parsed.doc) {
    return { ok: false, errors: parsed.errors, warnings: parsed.warnings, diff, summary: isi('{count} galat; tidak ada yang diterapkan.', { count: parsed.errors.length }) };
  }
  ...
  return {
    ...
    summary: [
      addedF.length ? `${addedF.length} ditambahkan` : '',
      removedF.length ? `${removedF.length} dihapus` : '',
