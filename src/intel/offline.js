 ...
 *
 * The prefix is `tessercad-id.`, not `tessercad.`. GitHub Pages serves this
 ...
 */
const KEYS = [
  ['tessercad-id.autosave.v3', 'dokumen yang sedang Anda buka'],
  ['tessercad-id.vcs.v1', 'versi dan cabang tersimpan'],
  ['tessercad-id.studio.v1', 'standar studio, keputusan, dan makro'],
  ['tessercad-id.prefs.v1', 'preferensi Anda'],
  ['tessercad-id.why.seen.v1', 'catatan teknik yang sudah Anda lihat'],
  ['tessercad-id.recentCommands', 'perintah yang terakhir Anda pakai'],
  ['tessercad-id.seenWelcome', 'apakah layar sambutan sudah tampil'],
];

/** Register the worker. Silent on failure: offline is a bonus, not a gate. */
export function install() {
  if (typeof navigator === 'undefined' || !navigator.serviceWorker) {
    return Promise.resolve({ ok: false, reason: 'Peramban ini tidak mendukung service worker.' });
  }
  ...
  if (typeof location !== 'undefined' && location.protocol === 'app:') {
    return Promise.resolve({ ok: false, reason: 'Sudah offline: ini build desktop, dan setiap berkas ada di lokal.' });
  }
  ...
  if (typeof location !== 'undefined' && location.protocol !== 'https:' && location.hostname !== 'localhost' && location.hostname !== '127.0.0.1') {
    return Promise.resolve({ ok: false, reason: 'Pemasangan offline butuh https, atau localhost.' });
  }
...
export async function status() {
  ...
  if (typeof caches !== 'undefined') {
    try {
      const keys = (await caches.keys()).filter(k => k.startsWith('tessercad-id-'));
...
export async function uninstall() {
  ...
  if (typeof caches !== 'undefined') {
    for (const k of await caches.keys()) {
      if (k.startsWith('tessercad-id-')) { await caches.delete(k); out.caches++; }
    }
  }
  const reg = await navigator.serviceWorker?.getRegistration?.('./');
...
export const NETWORK_FACTS = [
  'Tanpa akun, tanpa masuk, dan tidak ada yang perlu diaktivasi.',
  'Tanpa telemetri, tanpa analitik, dan tanpa pelaporan galat.',
  'Tanpa pemeriksaan lisensi, jadi tidak ada yang bisa menolak untuk mulai.',
  'Tidak ada font, peta, atau model yang diambil dari pihak lain: three.js disertakan di dalam repositori ini.',
  'Setiap dokumen yang Anda buka atau simpan berpindah antara halaman ini dan disk Anda, tidak ke mana pun lagi.',
  'Buka panel jaringan peramban Anda lalu muat ulang: setelah kunjungan pertama tidak ada apa pun yang terlihat.',
];
