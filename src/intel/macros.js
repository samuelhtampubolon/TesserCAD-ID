...
export class MacroRecorder {
  ...
  start(name) {
    this.recording = { name: name || 'Makro baru', steps: [], startedAt: Date.now(), skipped: [] };
    this.onChange();
  }
  ...
  run(macro) {
    ...
    if (!cmds.length) return { ok: false, ran: 0, failed: [], reason: 'Tidak satu pun perintahnya masih ada.' };

    const failed = [];
    let ran = 0;
    store.batch(`Makro: ${macro.name}`, () => {
      for (const c of cmds) {
        try {
          if (c.enabled && !c.enabled()) { failed.push({ id: c.id, why: 'tidak tersedia di sini' }); continue; }
