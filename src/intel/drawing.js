 ...
 */
export const VIEWS = {
  front: { label: 'DEPAN', dir: [0, 1, 0], up: [0, 0, 1] },
  top: { label: 'ATAS', dir: [0, 0, -1], up: [0, 1, 0] },
  right: { label: 'KANAN', dir: [-1, 0, 0], up: [0, 0, 1] },
...
export const SHEETS = {
  a4l: { label: 'A4 mendatar', w: 297, h: 210 },
  a3l: { label: 'A3 mendatar', w: 420, h: 297 },
  a2l: { label: 'A2 mendatar', w: 594, h: 420 },
};
...
function titleSVG(s, ink, thin) {
  ...
  L.push(cell(cols[0], y0, 0, ('BAGIAN'), t.name, true));
  L.push(cell(cols[1], y0, 0, ('MATERIAL'), t.material));
  L.push(cell(cols[2], y0, 0, ('MASSA'), `${t.mass.toFixed(3)} kg`));
  L.push(cell(cols[3], y0, 0, ('SKALA'), t.scale));
  L.push(cell(cols[4], y0, 0, ('SATUAN'), t.units));

  const y1 = y0 + h / 2;
  L.push(cell(cols[0], y1, 0, ('VARIAN'), t.variant || ('Bawaan')));
  L.push(cell(cols[1], y1, 0, ('PROSES'), t.process));
  L.push(cell(cols[2], y1, 0, ('TOL. UMUM'), `±${t.tolerance}`));
  L.push(cell(cols[3], y1, 0, ('PROYEKSI'), t.projection));
  L.push(cell(cols[4], y1, 0, ('TANGGAL'), t.date));

  L.push(`<text x="${f(x0 + w - 2)}" y="${f(y0 - 2)}" font-size="2.1" fill="${thin}" font-family="system-ui,sans-serif" text-anchor="end">${esc(t.generator)}${t.author ? ` · ${esc(t.author)}` : ''}${s.sampledHLR ? ' · garis tersembunyi disampel' : ''}</text>`);
  L.push('</g>');
  return L.join('');
}
...
export function sheetToDraw(s) {
  const layers = [
    { id: 'lv', name: 'TERLIHAT', color: '#e8eef7', visible: true, locked: false, weight: 2, style: 'solid' },
    { id: 'lh', name: 'TERSEMBUNYI', color: '#7b8797', visible: true, locked: false, weight: 1, style: 'dashed' },
    { id: 'lc', name: 'SUMBU', color: '#4da3ff', visible: true, locked: false, weight: 1, style: 'dashed' },
    { id: 'ld', name: 'DIMENSI', color: '#ffb454', visible: true, locked: false, weight: 1, style: 'solid' },
    { id: 'lt', name: 'JUDUL', color: '#c9d3e0', visible: true, locked: false, weight: 1, style: 'solid' },
  ];
  const entities = [];
  let n = 0;
