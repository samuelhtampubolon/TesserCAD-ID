...
const KEY = 'tessercad-id.studio.v1';
...
export function exportStudio() {
  return JSON.stringify({ app: 'TesserCAD-ID', kind: 'studio', version: 1, ...read() }, null, 2);
}

export function importStudio(json) {
  const raw = typeof json === 'string' ? JSON.parse(json) : json;
  if (raw.kind !== 'studio') throw new Error('Bukan berkas studio TesserCAD');
  const next = {
    standards: { ...DEFAULT_STANDARDS, ...(raw.standards || {}) },
