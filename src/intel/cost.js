...
export function costOne(part, processId, { batch = 1, rates = {} } = {}) {
  ...
  const drivers = [
    ...
    { key: 'machine', label: 'Waktu mesin', value: machine },
    { key: 'setup', label: 'Setup, per part', value: setup },
    { key: 'tooling', label: 'Perkakas, per part', value: tooling },
...
export function levers(part, { batch = 1, rates = {} } = {}) {
  ...
  test('Kecilkan kotak batas 10%', 'Permesinan ditagih atas balok awalnya, jadi selubungnya lebih menentukan daripada partnya.',
    (a) => { a.box = { x: a.box.x * 0.9, y: a.box.y * 0.9, z: a.box.z * 0.9 }; a.volume *= 0.729; });
  test('Kosongkan part sampai 60% volumenya', 'Rusuk dan kulit menahan kekakuan yang sama dengan material jauh lebih sedikit.',
    (a) => { a.volume *= 0.6; });
  if (part.material !== 'aluminium' && MATERIAL_PRICE[part.material] > MATERIAL_PRICE.aluminium) {
    test('Ganti ke aluminium', 'Lebih murah per kilogram dan lebih cepat dipotong daripada kebanyakan yang digantikannya.',
  ...
  if (batch === 1) {
    ...
    if (ten.best && ten.best.each < base.best.each * 0.85) {
      out.push({
        label: 'Pesan sepuluh, bukan satu',
        note: 'Setup dan tooling adalah biaya sekali bayar; pada batch satu, Anda menanggung semuanya sendiri.',
