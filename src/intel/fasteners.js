...
export const CLASSES = {
  '4.6': { proof: 225, tensile: 400, note: 'Baja lunak. Pengikat umum di mana tidak ada yang dihitung.' },
  '8.8': { proof: 580, tensile: 800, note: 'Baut struktural bawaan. Sebagian besar rakitan mesin.' },
  '10.9': { proof: 830, tensile: 1040, note: 'Tarik tinggi. Perlu torsi terkontrol dan washer yang dikeraskan.' },
  '12.9': { proof: 970, tensile: 1220, note: 'Kelas umum tertinggi. Getas bila disalahgunakan; bukan untuk beban kejut.' },
  A2: { proof: 210, tensile: 500, note: 'Stainless A2. Tahan korosi, bukan kekuatan. Macet bila dijalankan kering.' },
  A4: { proof: 240, tensile: 600, note: 'Stainless A4. Kelautan. Peringatan macet sama seperti A2.' },
};
...
export function spec(size, { cls = '8.8', length = null, fit = 'medium', lubricated = false } = {}) {
  ...
  return {
    ...
    standard: 'Baut socket head cap ISO 4762',
    diameter: m.d,
    pitch: m.pitch,
    tensileArea: m.area,
    proofStress: c.proof,
    ...
    torqueBasis: isi('T = K·F·d dengan K = {K} ({p1}), F = 90% proof load. Tabel torsi terbitan memakai rumus yang sama; koefisien gesek adalah bagian yang tidak bisa dijanjikan siapa pun.', { K, p1: lubricated ? 'lubricated' : 'ulir polos kering' }),
    clearanceHole: m.clear[FITS[fit] ?? 1],
    clearanceFit: fit,
    tappingDrill: m.tap,
    minThreadEngagement: Math.round(m.d * 10) / 10,
    engagementNote: 'Satu diameter kedalaman ulir di baja, dua di aluminium, tiga di plastik. Kurang dari itu ulirnya slek sebelum bautnya leleh, dan itu kegagalan yang salah.',
...
export function checkJoint(size, { cls = '8.8', load = 1000, count = 1, shear = false, safety = 2, lubricated = false } = {}) {
  ...
  return {
    ...
    utilisation: util,
    pass: util <= 1,
    verdict: util <= 0.5 ? 'Lega' : util <= 1 ? 'Cukup' : util <= 1.5 ? 'Melebihi kapasitas' : 'Jauh melebihi kapasitas',
    mode: shear ? 'shear' : 'tension',
    caveat: 'Orde pertama saja: baut dianggap terbebani sepanjang sumbunya tanpa lentur, tanpa eksentrisitas, tanpa kelelahan, dan tanpa relaksasi sambungan. Sambungan baut biasanya gagal di ulir, di material yang dijepit, atau karena kelelahan jauh sebelum bautnya mencapai proof load.',
  };
}
...
export function featuresFor(size, { length = 20, cls = '8.8' } = {}, makeFeature) {
  ...
  const shank = makeFeature('cylinder', {
    name: `${size}×${length} batang`, material: /^A/.test(cls) ? 'stainless' : 'steel',
    params: { r: s.diameter / 2, h: length, seg: 32, arc: 360 },
  });
  const head = makeFeature('cylinder', {
    name: `${size} kepala`, material: /^A/.test(cls) ? 'stainless' : 'steel',
...
export function bomRow(size, { cls = '8.8', length = 20, qty = 1 } = {}) {
  ...
  return {
    item: s.designation,
    description: isi('Baut socket head cap {size}×{length}, kelas {cls}, ISO 4762', { size, length, cls }),
    qty,
    diameter: s.diameter,
    pitch: s.pitch,
    torqueNm: s.torqueNm,
...
export function fastenerCSV(rows) {
  const head = ['No.', 'Keterangan', 'Qty', 'Dia mm', 'Pitch mm', 'Torsi Nm', 'Kelonggaran mm', 'Bor tap mm', 'Unit', 'Baris', 'Standar'];
