...
export function recognise(geometry, matrix = null, {
  ...
  if (tris.length > maxTriangles) {
    return {
      ...
      reason: isi('{n}k segitiga melewati batas analisis {limit}k.', { n: Math.round(tris.length / 1000), limit: maxTriangles / 1000 }),
    };
  }
...
export function cutterFor(hole, { depth = null, clearance: extra = 0, through = true } = {}) {
  ...
  return {
    type: 'cylinder',
    name: hole.nominal ? `Lubang ${hole.nominal.label}` : `Lubang Ø${(r * 2).toFixed(2)}`,
