...
export const FITS = {
  'H11/c11': { kind: 'clearance', label: 'Jalan longgar', note: 'Kelonggaran lebar untuk kotoran, cat, atau panas. Pakai di mana tidak ada yang harus terposisi.' },
  'H9/d9':   { kind: 'clearance', label: 'Jalan bebas',  note: 'Berputar cepat dengan pelumasan berlimpah; bukan untuk posisi yang akurat.' },
  'H8/f7':   { kind: 'clearance', label: 'Jalan rapat', note: 'Berputar bebas, memposisikan sedang. Suaian bantalan luncur bawaan.' },
  'H7/g6':   { kind: 'clearance', label: 'Geser',       note: 'Meluncur dan berputar dengan tangan, memposisikan akurat. Cocok untuk spigot.' },
  'H7/h6':   { kind: 'clearance', label: 'Lokasi longgar', note: 'Terpasang bebas dengan tangan dan tetap diam. Bawaan yang aman.' },
  'H7/k6':   { kind: 'transition', label: 'Lokasi transisi', note: 'Ketukan ringan untuk memasang. Posisi akurat tanpa press.' },
  'H7/n6':   { kind: 'transition', label: 'Lokasi interferensi', note: 'Ditekan untuk memasang. Posisi kaku, masih bisa dilepas.' },
  'H7/p6':   { kind: 'interference', label: 'Suaian tekan', note: 'Ditekan atau disusutkan. Torsi lewat sambungan hanya dengan pasak.' },
  'H7/s6':   { kind: 'interference', label: 'Suaian pukul', note: 'Press berat atau panas. Permanen; part jarang bisa dilepas tanpa rusak.' },
};
...
export const DISTRIBUTIONS = {
  ...
  sixSigma:   { label: 'Mampu (±6σ)', sigma: t => t / 6 },
  triangular: { label: 'Segitiga', sigma: t => t / Math.sqrt(6) },
  uniform:    { label: 'Seragam', sigma: t => t / Math.sqrt(3) },
};
...
export function cpkVerdict(cpk) {
  if (!Number.isFinite(cpk)) return { grade: 'exact', label: 'Tanpa variasi dimodelkan', severity: 'note' };
  if (cpk >= 2.00) return { grade: 'six-sigma', label: 'Six sigma: praktis tidak pernah gagal', severity: 'ok' };
  if (cpk >= 1.67) return { grade: 'excellent', label: 'Sangat baik: aman untuk perakitan otomatis', severity: 'ok' };
  if (cpk >= 1.33) return { grade: 'capable', label: 'Mampu: target produksi yang biasa', severity: 'ok' };
  if (cpk >= 1.00) return { grade: 'marginal', label: 'Marginal: siapkan sortir dan rework', severity: 'warn' };
  ...
  if (cpk >= 0.66) return { grade: 'poor', label: 'Buruk: beberapa persen tidak akan terpasang', severity: 'warn' };
  return { grade: 'incapable', label: 'Tidak mampu: rantai ini tidak memenuhi', severity: 'block' };
}
...
export function levers(stack, { target = 1.33, trials = 4000 } = {}) {
  ...
  if (!(worstHalfSpace > 0)) {
    ...
    out.nominal = {
      ...
      note: isi(mu > upper
      ? 'Rantai nominal menutup di {mu} mm, yaitu {miss} mm di atas kebutuhan. Memperketat toleransi tidak bisa mencapai angka yang nominalnya sendiri tidak pernah jumlahkan: ubah sebuah dimensi sebesar {delta} mm, atau periksa arah setiap tautan sudah benar.'
      : 'Rantai nominal menutup di {mu} mm, yaitu {miss} mm di bawah kebutuhan. Memperketat toleransi tidak bisa mencapai angka yang nominalnya sendiri tidak pernah jumlahkan: ubah sebuah dimensi sebesar {delta} mm, atau periksa arah setiap tautan sudah benar.',
  ...
  if (base.capability.sigma > 0 && worstHalfSpace > 0) {
    ...
    out.uniform = wantedOpenVar <= 0
      ? { possible: false, note: 'Bahkan toleransi nol pada tautan terbuka tidak mencapai target: tautan tetapnya saja sudah terlalu longgar.' }
  ...
  if (base.capability.cp >= target && base.capability.cpk < target) {
    out.centring = { move: (lower + upper) / 2 - mu, note: 'Rantainya cukup ketat tetapi melenceng dari pusat. Geser sebuah nominal alih-alih membeli toleransi.' };
  }
  return out;
}
...
export function allocate(stack, { method = 'proportional', target = 1.33, floor = 0.005 } = {}) {
  ...
  if (!(room > 0)) return links.map(l => ({ id: l.id, label: l.label, tol: null, note: 'Rantai nominalnya sudah meleset dari kebutuhan; perbaiki nominalnya dulu.' }));
  ...
  return links.map((l, i) => {
    ...
    return {
      ...
      tight: tol < floor,
      note: tol < floor ? isi('Butuh ±{p1}mm, di bawah batas bawah ±{floor}mm. Pikirkan ulang rantainya, bukan toleransinya.', { p1: tol.toFixed(4), floor }) : '',
    };
  });
}
...
export function stackFromBuild(bodies, { axis = 'x', process = null, limits = null, requirement = 'Assembly gap' } = {}) {
  ...
  for (const b of bodies || []) {
    ...
    links.push(makeLink({
      label: `${b.feature?.name || 'Badan'} ${axis.toUpperCase()}`,
...
export function stackSummary(stack) {
  ...
  return {
    ...
    spec: isi('{p1} sampai {p2} mm', { p1: a.lower.toFixed(3), p2: a.upper.toFixed(3) }),
    worstCase: isi('{p1} sampai {p2} mm', { p1: a.worst.min.toFixed(3), p2: a.worst.max.toFixed(3) }),
