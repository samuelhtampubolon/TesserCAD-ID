...
function readableError(err) {
  if (err?.code === 'TRI_BUDGET') {
    return isi('Boolean dilewati: {k}k segitiga melebihi anggaran {budget}k. Turunkan jumlah segmen pada masukannya.', err.data);
  }
...
function evalOne(feature, ctx) {
  ...
  for (const id of feature.inputs) {
    ...
    if (!r) throw new Error('Fitur masukan tidak ada');
    if (r.error) throw new Error(isi('Masukan "{name}" gagal', { name: r.name }));
  ...
  switch (feature.type) {
    ...
    case 'patternLinear': {
      if (!inputInstances.length) throw new Error('Pattern linear butuh body masukan');
      const n1 = Math.max(1, Math.round(params.count));
      const n2 = Math.max(1, Math.round(params.count2));
      if (n1 * n2 > 2000) throw new Error('Pattern akan membuat lebih dari 2000 salinan');
      const out = [];
      for (let i = 0; i < n1; i++) {
        for (let j = 0; j < n2; j++) {
    ...
    case 'patternCircular': {
      if (!inputInstances.length) throw new Error('Pattern melingkar butuh body masukan');
      const n = Math.max(1, Math.round(params.count));
      if (n > 2000) throw new Error('Pattern akan membuat lebih dari 2000 salinan');
    ...
    case 'mirror': {
      if (!inputInstances.length) throw new Error('Mirror butuh body masukan');
    ...
    case 'mesh': {
      ...
      if (!data || !data.positions || !data.positions.length) throw new Error('Mesh impor tidak punya geometri');
