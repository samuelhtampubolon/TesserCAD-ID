...
export function buildPrimitive(type, p) {
  switch (type) {
    ...
    case 'tube': {
      ...
      if (ri >= ro) throw new Error('Radius dalam tabung harus lebih kecil dari radius luar');
...
export function buildExtrude(shapes, opts = {}) {
  ...
  if (Math.abs(dist) < 1e-6) throw new Error('Jarak extrude tidak boleh nol');
  ...
  if (!positions.length) throw new Error('Tidak ditemukan profil tertutup untuk di-extrude');
...
export function buildRevolve(shapes, opts = {}) {
  ...
  if (Math.abs(angle) < 1e-6) throw new Error('Sudut revolve tidak boleh nol');
  if (!shapes.length) throw new Error('Tidak ditemukan profil tertutup untuk di-revolve');
  ...
  for (const shape of shapes) {
    ...
    for (const ring of rings) {
      ...
      if (minU < -1e-3) throw new Error('Profil revolve harus tetap di satu sisi sumbu');
  ...
  if (!positions.length) throw new Error('Revolve tidak menghasilkan geometri');
  const g = new THREE.BufferGeometry();
  g.setAttribute('position', new THREE.BufferAttribute(new Float32Array(positions), 3));
  g.computeVertexNormals();
  return g;
