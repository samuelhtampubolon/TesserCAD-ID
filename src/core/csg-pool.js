...
class Pool {
  ...
  start() {
    ...
    if (typeof Worker === 'undefined') {
      this.available = false; this.reason = 'Peramban ini tidak mendukung Worker.';
      return false;
    }
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        w.onerror = (err) => { this._collapse(err?.message || 'sebuah worker gagal dimuat'); };
        this.workers.push(w);
        this.idle.push(w);
      }
      this.available = true;
      ...
      this.reason = err.message || 'worker tidak bisa dijalankan';
      return false;
    }
  }
