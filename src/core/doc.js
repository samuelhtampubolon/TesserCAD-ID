...
export const APP_NAME = 'TesserCAD-ID';
...
export const MATERIALS = {
  ...
  brass:     { name: 'Kuningan',        density: 8.50e-6, color: '#d7a94b', metal: 0.95, rough: 0.30 },
  copper:    { name: 'Tembaga',       density: 8.96e-6, color: '#c9764a', metal: 0.95, rough: 0.28 },
  titanium:  { name: 'Titanium',     density: 4.50e-6, color: '#9aa0a6', metal: 0.85, rough: 0.42 },
  abs:       { name: 'Plastik ABS',  density: 1.04e-6, color: '#e8e2d8', metal: 0.0, rough: 0.62 },
  pla:       { name: 'PLA',          density: 1.24e-6, color: '#6fcf97', metal: 0.0, rough: 0.58 },
  nylon:     { name: 'Nilon',        density: 1.15e-6, color: '#f2f2f2', metal: 0.0, rough: 0.70 },
  acrylic:   { name: 'Akrilik',      density: 1.18e-6, color: '#bfe6ff', metal: 0.0, rough: 0.10 },
  wood:      { name: 'Kayu (pinus)',  density: 0.50e-6, color: '#c08b5c', metal: 0.0, rough: 0.80 },
  concrete:  { name: 'Beton',     density: 2.40e-6, color: '#a5a5a0', metal: 0.0, rough: 0.92 },
...
export const CATALOG = {
  box: {
    ...
    fields: [
      { key: 'w', label: 'Lebar (X)', kind: 'len' },
      { key: 'd', label: 'Dalam (Y)', kind: 'len' },
      { key: 'h', label: 'Tinggi (Z)', kind: 'len' },
    ],
  },
  cylinder: {
    ...
    fields: [
      ...
      { key: 'h', label: 'Tinggi', kind: 'len' },
      { key: 'arc', label: 'Sweep', kind: 'ang' },
      { key: 'seg', label: 'Segmen', kind: 'int', min: 3, max: 256 },
    ],
  },
  sphere: {
    ...
    fields: [
      { key: 'r', label: 'Radius', kind: 'len' },
      { key: 'seg', label: 'Segmen', kind: 'int', min: 4, max: 200 },
    ],
  },
  cone: {
    ...
    fields: [
      { key: 'r1', label: 'R bawah', kind: 'len' },
      { key: 'r2', label: 'R atas', kind: 'len' },
      { key: 'h', label: 'Tinggi', kind: 'len' },
      { key: 'seg', label: 'Segmen', kind: 'int', min: 3, max: 256 },
    ],
  },
  torus: {
    ...
    fields: [
      { key: 'R', label: 'R cincin', kind: 'len' },
      { key: 'r', label: 'R tabung', kind: 'len' },
      { key: 'arc', label: 'Sweep', kind: 'ang' },
      { key: 'seg', label: 'Seg. cincin', kind: 'int', min: 3, max: 256 },
      { key: 'tseg', label: 'Seg. tabung', kind: 'int', min: 3, max: 128 },
    ],
  },
  tube: {
    label: 'Tube / Pipa', glyph: '◯', group: 'solid',
    params: { ro: 22, ri: 15, h: 60, seg: 48 },
    fields: [
      { key: 'ro', label: 'R luar', kind: 'len' },
      { key: 'ri', label: 'R dalam', kind: 'len' },
      { key: 'h', label: 'Tinggi', kind: 'len' },
      { key: 'seg', label: 'Segmen', kind: 'int', min: 3, max: 256 },
    ],
  },
  ...
  prism: {
    ...
    fields: [
      { key: 'r', label: 'Radius', kind: 'len' },
      { key: 'h', label: 'Tinggi', kind: 'len' },
      { key: 'sides', label: 'Sisi', kind: 'int', min: 3, max: 64 },
    ],
  },
  pyramid: {
    ...
    fields: [
      { key: 'r', label: 'R alas', kind: 'len' },
      { key: 'h', label: 'Tinggi', kind: 'len' },
      { key: 'sides', label: 'Sisi', kind: 'int', min: 3, max: 64 },
    ],
  },
  plate: {
    label: 'Pelat sudut bulat', glyph: '▭', group: 'solid',
    params: { w: 80, d: 50, h: 8, fillet: 8, hole: 0, seg: 12 },
    fields: [
      { key: 'w', label: 'Lebar (X)', kind: 'len' },
      { key: 'd', label: 'Dalam (Y)', kind: 'len' },
      { key: 'h', label: 'Tebal', kind: 'len' },
      { key: 'fillet', label: 'R sudut', kind: 'len' },
      { key: 'hole', label: 'R lubang pusat', kind: 'len' },
      { key: 'seg', label: 'Seg. sudut', kind: 'int', min: 1, max: 48 },
    ],
  },
  helix: {
    label: 'Helix / Pegas', glyph: '⌇', group: 'solid',
    params: { R: 25, r: 4, pitch: 14, turns: 6, seg: 24, steps: 24 },
    fields: [
      { key: 'R', label: 'R koil', kind: 'len' },
      { key: 'r', label: 'R kawat', kind: 'len' },
      { key: 'pitch', label: 'Pitch', kind: 'len' },
      { key: 'turns', label: 'Lilitan', kind: 'num', min: 0.25, max: 200 },
      { key: 'seg', label: 'Seg. kawat', kind: 'int', min: 3, max: 48 },
      { key: 'steps', label: 'Langkah/lilit', kind: 'int', min: 6, max: 96 },
    ],
  },
  ...
  extrude: {
    ...
    fields: [
      { key: 'dist', label: 'Jarak', kind: 'len' },
      { key: 'symmetric', label: 'Bidang tengah', kind: 'bool' },
      { key: 'plane', label: 'Bidang sketch', kind: 'select', options: [['xy', 'XY (atas)'], ['xz', 'XZ (depan)'], ['yz', 'YZ (kanan)']] },
      { key: 'offset', label: 'Offset bidang', kind: 'len' },
      { key: 'taper', label: 'Sudut draft', kind: 'ang', min: -60, max: 60 },
      { key: 'twist', label: 'Twist', kind: 'ang', min: -1440, max: 1440 },
      { key: 'steps', label: 'Langkah', kind: 'int', min: 1, max: 200 },
    ],
  },
  revolve: {
    ...
    fields: [
      { key: 'angle', label: 'Sudut', kind: 'ang', min: -360, max: 360 },
      { key: 'axis', label: 'Sumbu', kind: 'select', options: [['y', 'Vertikal (sketch Y)'], ['x', 'Horizontal (sketch X)']] },
      { key: 'plane', label: 'Bidang sketch', kind: 'select', options: [['xz', 'XZ (depan)'], ['xy', 'XY (atas)'], ['yz', 'YZ (kanan)']] },
      { key: 'seg', label: 'Segmen', kind: 'int', min: 3, max: 360 },
    ],
  },
  ...
  boolean: {
    ...
    params: { op: 'union' },
    fields: [{ key: 'op', label: 'Operasi', kind: 'select', options: [['union', 'Union (tambah)'], ['subtract', 'Subtract (potong)'], ['intersect', 'Intersect (irisan)']] }],
  },
  patternLinear: {
    ...
    fields: [
      { key: 'count', label: 'Jumlah 1', kind: 'int', min: 1, max: 400 },
      { key: 'dx', label: 'Langkah 1 X', kind: 'len' },
      { key: 'dy', label: 'Langkah 1 Y', kind: 'len' },
      { key: 'dz', label: 'Langkah 1 Z', kind: 'len' },
      { key: 'count2', label: 'Jumlah 2', kind: 'int', min: 1, max: 400 },
      { key: 'dx2', label: 'Langkah 2 X', kind: 'len' },
      { key: 'dy2', label: 'Langkah 2 Y', kind: 'len' },
      { key: 'dz2', label: 'Langkah 2 Z', kind: 'len' },
    ],
  },
  patternCircular: {
    ...
    fields: [
      { key: 'count', label: 'Jumlah', kind: 'int', min: 1, max: 400 },
      { key: 'angle', label: 'Sudut total', kind: 'ang', min: -360, max: 360 },
      { key: 'axis', label: 'Sumbu', kind: 'select', options: [['x', 'X'], ['y', 'Y'], ['z', 'Z']] },
      { key: 'cx', label: 'Pusat X', kind: 'len' },
      { key: 'cy', label: 'Pusat Y', kind: 'len' },
      { key: 'cz', label: 'Pusat Z', kind: 'len' },
      { key: 'rotate', label: 'Putar salinan', kind: 'bool' },
    ],
  },
  mirror: {
    ...
    fields: [
      { key: 'plane', label: 'Bidang mirror', kind: 'select', options: [['yz', 'YZ (mirror X)'], ['xz', 'XZ (mirror Y)'], ['xy', 'XY (mirror Z)']] },
      { key: 'offset', label: 'Offset bidang', kind: 'len' },
      { key: 'keep', label: 'Pertahankan asli', kind: 'bool' },
    ],
  },
  mesh: {
    label: 'Mesh impor', glyph: '◇', group: 'import',
    params: { scale: 1 },
...
export function emptyDraw() {
  ...
  const dims = makeLayer('Dimensi', '#4da3ff');
...
export function newDocument(name = 'Tanpa judul') {
  return {
    schema: SCHEMA,
    app: APP_NAME,
...
export function migrate(doc) {
  if (!doc || typeof doc !== 'object') throw new Error('Bukan dokumen TesserCAD');
  ...
  if (!d.configs || !Array.isArray(d.configs.list) || !d.configs.list.length) {
    d.configs = { active: 'default', list: [{ id: 'default', name: 'Bawaan', overrides: {}, note: 'Desain sebagaimana digambar.' }] };
  }
...
class Store {
  constructor() {
    ...
    this.root = historyNode('Dokumen kosong', this.doc);
    this.head = this.root;
    this._clock = nodeSeq;
    this.dirty = false;
    this._label = null;
  ...
  commit({ rebuild = true, silent = false } = {}) {
    if (this._batch) {
      ...
      const node = historyNode(this._label || 'Sunting', this.doc, this.head);
  ...
  _prune() {
    ...
    if (spineList.length > HISTORY_LIMIT) {
      ...
      newRoot.parent = null;
      newRoot.label = 'Riwayat sebelumnya dibuang';
      this.root = newRoot;
    }
  }
  ...
  gotoNode(id) {
    ...
    return this._goto(found, 'Lompat ke');
  }
...
const AUTOSAVE_KEY = 'tessercad-id.autosave.v3';
