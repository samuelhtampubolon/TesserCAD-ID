...
function parse(tokens, scope) {
  ...
  function term() {
    ...
    while (peek() && (peek().t === '*' || peek().t === '/' || peek().t === '%')) {
      ...
      const r = unary();
      if (op === '*') v = v * r;
      else if (op === '/') { if (r === 0) throw new Error('Pembagian dengan nol'); v = v / r; }
      else { if (r === 0) throw new Error('Modulo dengan nol'); v = v % r; }
    }
    return v;
  }
  function unary() {
  ...
  function atom() {
    const tk = peek();
    if (!tk) throw new Error('Ekspresi berakhir tak terduga');
    ...
    if (tk.t === 'id') {
      ...
      if (Object.prototype.hasOwnProperty.call(scope, name)) {
        const v = scope[name];
        if (typeof v !== 'number' || !Number.isFinite(v)) throw new Error(isi('Parameter "{name}" bukan angka terhingga', { name }));
        return v;
      }
  ...
  const value = expr();
  if (p !== tokens.length) throw new Error('Ada karakter sisa di ekspresi');
  return value;
}
...
export function evaluate(src, scope = {}) {
  if (typeof src === 'number') {
    if (!Number.isFinite(src)) throw new Error('Nilai tidak terhingga');
    return src;
  }
  if (src == null || src === '') throw new Error('Empty value');
  const v = parse(tokenize(String(src)), scope);
  if (!Number.isFinite(v)) throw new Error('Ekspresi menghasilkan nilai tak terhingga');
  return v;
}
...
export function tryEval(src, scope = {}) {
  try { return { ok: true, value: evaluate(src, scope), error: null }; }
    ...
    const message = e instanceof RangeError || /call stack/i.test(e.message || '')
      ? 'Ekspresi bersarang terlalu dalam untuk dihitung. Pecah menjadi dua parameter.'
      : e.message;
