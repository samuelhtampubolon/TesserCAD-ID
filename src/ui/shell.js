...
export function el(tag, attrs = {}, children = []) {
  ...
  for (const [k, v] of Object.entries(attrs)) {
    ...
    else if (k === 'title' || k === 'placeholder' || k === 'aria-label' || k === 'aria-placeholder') node.setAttribute(k, (v));
...
export function modal({ title, subtitle, body, actions = [], wide = false, size = '', onClose = null, icon: ic = null }) {
  ...
  box.append(
    el('div', { class: 'modal-head' }, [
      ...
      el('button', { class: 'mini-btn', title: 'Tutup', 'aria-label': 'Tutup', onclick: () => closeModal() }, [icon('close', { size: 15 })]),
    ]),
    bodyNode,
  );
...
export function confirmDialog(title, message, onYes, { danger = false, yes = 'Konfirmasi', icon: ic = 'warning' } = {}) {
  modal({
    title, icon: ic,
    body: [el('p', { text: message })],
    actions: [{ label: 'Batal' }, { label: yes, primary: !danger, danger, run: () => { onYes(); } }],
  });
}
...
export function promptDialog(title, label, value, onOk, { placeholder = '', help = '', icon: ic = 'rename' } = {}) {
  ...
  modal({
    ...
    actions: [{ label: 'Batal' }, { label: 'OK', primary: true, run: () => onOk(input.value) }],
  });
...
const RECENT_KEY = 'tessercad-id.recentCommands';
...
export function commandPalette(commands, onRun, { context = '' } = {}) {
  ...
  const input = el('input', { type: 'text', placeholder: 'Cari perintah…   coba "extrude", "export stl", "gelap"', spellcheck: 'false', 'aria-label': 'Cari perintah' });
  const list = el('ul', { role: 'listbox' });
  const foot = el('div', { class: 'palette-foot' }, [
    el('span', {}, [el('kbd', { text: '↑↓' }), ' navigasi']),
    el('span', {}, [el('kbd', { text: '⏎' }), ' jalankan']),
    el('span', {}, [el('kbd', { text: 'esc' }), ' tutup']),
  ...
  let heading = recent.length ? ('Terakhir dipakai') : ('Semua perintah');
  let cursor = 0;
  ...
  const render = () => {
    ...
    if (!filtered.length) {
      list.appendChild(el('li', { class: 'palette-empty' }, [
        icon('search', { size: 18 }), el('span', { text: 'Tidak ada perintah yang cocok' }),
      ]));
    }
  ...
  input.addEventListener('input', () => {
    ...
    if (!q) { filtered = recent.length ? recent : commands; heading = recent.length ? ('Terakhir dipakai') : ('Semua perintah'); }
    else {
      filtered = commands
        .map(c => ({ c, s: score(c, q) }))
        .filter(x => x.s >= 0)
        .sort((a, b) => b.s - a.s)
        .map(x => x.c);
      heading = isi('{n} hasil', { n: filtered.length });
    }
    cursor = 0;
    render();
  });
...
export function scrubNumber(value, onChange, {
  ...
  const input = el('input', { type: 'text', class: 'scrub', value: fmtNum(value, precision), title: title || 'Seret kiri/kanan untuk mengubah, atau ketik nilai' });
  let drag = null;
