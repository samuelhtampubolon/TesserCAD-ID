...
export class MobileShell {
  ...
  _build() {
    this.bar = el('nav', { id: 'bottombar', role: 'tablist', 'aria-label': 'Workspace dan panel' });
  ...
  renderBar() {
    if (!this.barItems) {
      ...
      add('panels', 'Panel', 'panel-bottom', () => this.togglePanelSheet());
      add('more', 'Lainnya', 'dots', () => this.toggleMenuSheet());
    }
  ...
  renderFab() {
    ...
    if (this.fabMode !== mode) {
      ...
      if (tablet) add('dock', 'panel-right', 'Tampilkan atau sembunyikan panel samping', () => this.app.toggleDock());
      if (base === 'draft') {
        add('fit', 'fit', 'Zoom ke batas gambar', () => this.app.draft.zoomExtents());
        add('snap', 'magnet', 'Object snap', () => { this.app.toggleDraft('snap'); this.renderFab(); });
        add('undo', 'undo', 'Undo', () => store.undo());
      } else {
        add('fit', 'fit', 'Zoom pas', () => this.app.zoomFit());
        ...
        add('view', 'view-iso', 'Sudut pandang dan tampilan', () => (isTablet()
  ...
  openSheet({ kind, title, content, tabs = null, tall = false }) {
    ...
    const head = el('div', { class: 'sheet-head' }, [
      ...
      el('button', { class: 'sheet-close', 'aria-label': 'Tutup', onclick: () => this.closeSheet() }, [icon('close', { size: 18 })]),
    ]);
  ...
  openPanelSheet(which) {
    ...
    const tabs = el('div', { class: 'sheet-tabs' }, [
      ...
      }, [icon('workspace', { size: 15 }), el('span', { text: this.app.workspace === 'draft' ? 'Layer' : 'Outline' })]),
      el('button', {
        class: which === 'right' ? 'on' : '', onclick: () => this.openPanelSheet('right'),
      }, [icon('settings', { size: 15 }), el('span', { text: 'Properti' })]),
    ]);

    this.openSheet({ kind: 'panel', title: 'Panel', content: node, tabs });
  ...
  openMenuSheet() {
    ...
    }, [icon('search', { size: 17 }), el('span', { text: isi('Cari semua {n} perintah…', { n: app.commands.length }) }), el('kbd', { text: '⌘K' })]));
    ...
    this.openSheet({ kind: 'menu', title: 'Semua perintah', content: wrap, tall: true });
  }
  ...
  openViewMenu(anchor) {
    ...
    dropdown(anchor, [
      ...grp('Tampilan standar', ['view.iso', 'view.front', 'view.right', 'view.top', 'view.back', 'view.left']),
      '-',
      ...grp('Tampilan', ['shade.shaded-edges', 'shade.shaded', 'shade.wire', 'shade.xray', 'view.ortho', 'view.grid']),
      '-',
  ...
  openViewSheet() {
    ...
    grid('Tampilan standar', ['view.iso', 'view.front', 'view.right', 'view.top', 'view.back', 'view.left']);
    grid('Tampilan', ['shade.shaded-edges', 'shade.shaded', 'shade.wire', 'shade.xray', 'view.ortho', 'view.grid']);
    grid('Framing', ['view.fit', 'view.selection', 'mod.isolate', 'mod.showAll', 'view.section', 'view.theme']);
    this.openSheet({ kind: 'view', title: 'Tampilan', content: wrap });
  }
}

const SHORT = {
  'edit.undo': 'Undo', 'edit.redo': 'Redo', 'file.save': 'Simpan', 'file.template': 'Templates',
  'edit.prefs': 'Settings', 'help.quickstart': 'Bantuan',
  'view.iso': 'Iso', 'view.front': 'Depan', 'view.right': 'Kanan', 'view.top': 'Atas',
  'view.back': 'Belakang', 'view.left': 'Kiri',
  'shade.shaded-edges': 'Rusuk', 'shade.shaded': 'Solid', 'shade.wire': 'Kawat', 'shade.xray': 'X-ray',
  'view.ortho': 'Ortho', 'view.grid': 'Grid', 'view.fit': 'Muat semua', 'view.selection': 'Fit sel',
  'mod.isolate': 'Isolasi', 'mod.showAll': 'Tampil semua', 'view.section': 'Section', 'view.theme': 'Tema',
};
