...
export class OperatorHost {
  ...
  start(kind) {
    ...
    if (!ids.length) { this.app.flash('Pilih sebuah body dulu', 'warn'); return false; }
  ...
  _render() {
    ...
    const v = this._effectiveValue();
    const axisLabel = a.axis ? (a.plane ? `bidang ${a.axis.toUpperCase()}` : `${a.axis.toUpperCase()} axis`) : 'bidang tampilan';
    ...
    this.hud.append(
      ...
      row('op-hint', a.bodies.length > 1 ? `${a.bodies.length} body · ${HINT}` : HINT),
    );
    this.hud.classList.toggle('typing', !!a.typed);
  }
}

const LABELS = { move: 'Geser', rotate: 'Rotate', scale: 'Scale' };
const CARET = { move: ' mm', rotate: '°', scale: '×' };
const HINT = 'Sumbu X/Y/Z · ⇧X bidang · ketik angka · ⇧ presisi · ⌃ snap · ⏎ konfirmasi · esc batal';
