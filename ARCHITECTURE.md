...
- `intel` touches no DOM, which is why all twenty-two engineering modules are
  tested headlessly rather than through a browser. `ai` above it obeys the same
  rule for the same reason: both chat features are driven in Node by
  `tools/tests/chat3d.mjs` and `tools/tests/chat4d.mjs`, with no browser and no
  dialog.
...
Two modules that TesserCAD has are deliberately absent: `merge.js`, the
three-way merge over the feature tree, and the deviation-map half of
`deviation.js`. [COMPARISON.md](COMPARISON.md) records what each removal
recovered in payload and what it costs a user.
...
| `intent.js` | Design-intent JSON read back into a live document, and the round trip checked |
...
| | |
|---|---|
| `lex.js` | Indonesian numbers, sizes, counts, durations, and clause splitting |
| `resep.js` | Eleven parametric assemblies, as data: fields, defaults, builders |
| `chat3d.js` | The 3D conversation: recipe, single shape, edit, question, refusal |
| `chat4d.js` | The 4D conversation: sequence, motor, keyframe, physics |
| `klik.js` | What a plan would have cost with the mouse, as a published model |

The placement is the design. A recipe produces **catalogue features**, so it
...
whole feature - 231 of the 1095 headless checks - be tested without a browser,
including building all eleven assemblies and rebuilding each through the real
geometry engine.

It is also what keeps the honesty enforceable. There is no model and no
network: `ai` imports nothing but `core`, `intel/speak.js` and `sim`, and the
...
have to be, and comparing a 25,000-line browser application to them on
...
| Time to first test run | `npm test`, under half a minute, downloads nothing |
...
| Test checks | 1095 headless in 16 suites, 407 across 11 browser suites, 18 in the desktop shell |
