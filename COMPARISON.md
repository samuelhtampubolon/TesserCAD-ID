...
| Tests | 1026 headless in 16 suites, 407 across 11 browser suites, 18 in the real desktop shell |
...
- **A fifth layer in the architecture** (`ai`, above `intel`/`sim` and below
  ...
  testable in Node, and 231 of the 1026 headless checks are that.
...
denies every Electron permission. 83 security checks run attacks, not
...
```bash
npm test                            # 1026 checks, 16 suites
