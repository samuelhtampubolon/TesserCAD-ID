...
/**
 * The size promise, and what it costs.
 *
 * TesserCAD and TesserCADIna both choose a deflate zip at `compression:
 * normal` and refuse a self-extracting `portable` target, on the grounds that
 * solid LZMA and a self-extractor both look like a packed executable to some
 * endpoint protection and draw a warning on first run.
 *
 * This edition makes the opposite trade, deliberately, because its front page
 * promises a Windows download around 70 MB and under 80 MB and a deflate zip
 * of the same tree measures 135 MB. There is no configuration that gets a
 * deflate zip under 80 MB; the only levers are the compressor, the number of
 * Chromium locale packs, and the Electron version itself. Two of those are
 * pulled here and the third is left alone.
 *
 * So these three checks are inverted rather than deleted: the choice is the
 * opposite one, it is still a choice, and it is still asserted so that nobody
 * reverts it by accident and then wonders why the size gate went red. The
 * warning it costs is stated on the download page and in SECURITY.md, next to
 * the SHA-256 and the provenance attestation that are the real answer to
 * "is this file the one the workflow built".
 */
ok('the Windows download is LZMA, because deflate cannot meet the size promise',
