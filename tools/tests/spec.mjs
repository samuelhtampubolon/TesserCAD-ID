...
ok('but no feature is reported as changed, because none is', review.summary === 'Tidak ada perubahan', review.summary);
...
bad = S.fromSpec('part "X"\n  w = 10\n');
ok('an orphan indented line is caught', /tanpa fitur di atasnya/.test(bad.errors[0].message));
bad = S.fromSpec('part "X"\nfeature box "A"\n  nonsense\n');
...
bad = S.fromSpec('part "X"\nparam a\n');
ok('a parameter with no value is rejected', /butuh nilai/.test(bad.errors[0].message));
bad = S.fromSpec('part "X"\nfeature box "A" #x1\nfeature box "B" #x1\n');
