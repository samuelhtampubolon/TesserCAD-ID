...
ok('the class note explains what it is for', /struktural/.test(F.CLASSES['8.8'].note));
...
ok('and the note gives the real rule per material', /dua di aluminium/.test(s.engagementNote));
...
ok('and the verdict says so in words', j.verdict === 'Lega', j.verdict);
j = F.checkJoint('M4', { load: 40000, count: 2, safety: 2 });
ok('40 kN on two M4 fails', !j.pass && j.utilisation > 1, `${(j.utilisation * 100).toFixed(0)}%`);
ok('and is named as far over capacity', /melebihi kapasitas/i.test(j.verdict), j.verdict);
...
ok('every joint answer carries its caveat', /orde pertama/i.test(j.caveat) && /kelelahan/.test(j.caveat));
