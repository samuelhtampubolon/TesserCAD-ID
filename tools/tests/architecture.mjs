 ...
 *   3 intel   the engineering layer: doctor, cost, drawings, tolerance,
 ...
 *   6 main    the composition root, and the only file that imports ui.
 */
const LAYERS = [
  ['core'],
  ['draft', 'view'],
...
const aiFiles = files.filter(f => relative(srcRoot, f).startsWith('ai/'));
const domInAI = aiFiles.filter(f => touchesDOM(readFileSync(f, 'utf8')));
ok('no AI planner touches the DOM either, so both chats are testable in Node',
  domInAI.length === 0, domInAI.map(f => relative(srcRoot, f)).join(', '));
ok('the AI layer is a layer, not one file', aiFiles.length >= 4, `${aiFiles.length} modules`);
...
ok('no module mentions a prior-art project outside the three that explain why',
  unexpected.length === 0, unexpected.join(', '));
ok('and the count ATTRIBUTION.md publishes is the count there is',
  mentions.length === 4, `${mentions.length}: ${mentions.join(', ')}`);
