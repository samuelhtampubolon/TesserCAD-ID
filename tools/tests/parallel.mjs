...
ok('and says why', /tidak mendukung Worker/.test(pool.reason), pool.reason);
let rejected = null;
...
ok('a boolean with too few inputs reports the same error as the sync path',
  /setidaknya dua/.test(badRun.results.get(orphan.id).error || ''),
...
ok('an intersection of bodies that do not touch is reported, not silently empty',
  /body kosong/.test(emptyRun.results.get(nothing.id).error || ''),
