...
ok('the cache is versioned by content', /^tessercad-id-[0-9a-f]{12}$/.test(st.version || ''), st.version);
...
await page.waitForTimeout(500);
t = await modalText();
ok('the fastener panel gives the proof load', /Beban proof 21\.2 kN/.test(t), (t.match(/Beban proof [^ ]+ kN/) || [''])[0]);
ok('the torque, with its assumption stated', /30\.6 N·m/.test(t) && /K = 0\.2/.test(t));
ok('both hole sizes', /Lubang clearance ⌀9 mm/.test(t) && /Bor tap ⌀6\.8 mm/.test(t));
